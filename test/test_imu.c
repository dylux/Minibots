...
void pollIMU() {
    ...
    Gyroscope(g.gyro.z);  
    Accelerometer(g.acceleration.ux, g.acceleration.y, g.acceleration.z);
    delay(1);
    dt = (curtime - prevtime) / 1000000.0;
}
...
void Gyroscope(double x_gyro) {
    ...
    x_angular_rate[1] = x_gyro - ss_gyro_x;
...
void Accelerometer(double x_accel, double y_accel, double z_accel) {
    ...
    x_acceleration[1] = x_accel - ss_accel_x;
    y_acceleration[1] = y_accel - ss_accel_y;
    z_acceleration[1] = z_accel - ss_accel_z;
    ...
    //project acceleration into the global frame of reference: matrix and vector multiplication
    for (i=0; i<3; i++) {
        for (j=0; j<3; j++) {
...
void Calibrate(void) {
    ...
    do{ //accumulate samples
        ss_gyro_x = ss_gyro_x + g.gyro.z;
        ss_accel_x = ss_accel_x + g.acceleration.x;
        ss_accel_y = ss_accel_y + g.acceleration.y;
        ss_accel_z = ss_accel_z + g.acceleration.z;
        count1++;
